...
def _dispatch_ernc(body: "ERNCRequest") -> ERNCResponse:  # type: ignore[valid-type]
    """Despacha al cálculo según la topología."""
    try:
        if body.topologia == "string_dc":
            return calc_string_dc(body.datos)
        if body.topologia == "ac_inversor":
            return calc_ac_inversor(body.datos)
        if body.topologia == "gd_red_bt":
            return calc_gd_red_bt(body.datos)
        if body.topologia == "baterias_dc":
...
async def calc_ernc(
    body: ERNCRequest,  # type: ignore[valid-type]
    ...
    """
    return _dispatch_ernc(body)


@router.post("/ernc/public", response_model=ERNCResponse)
@limiter.limit("30/minute;500/hour")
async def calc_ernc_public(request: Request, body: ERNCRequest):  # type: ignore[valid-type]
    """Endpoint público ERNC (sin auth). Rate limit: 30/min, 500/hora."""
    return _dispatch_ernc(body)
